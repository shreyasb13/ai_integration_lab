...
if "client" not in st.session_state:
    ...
    if uploaded_file_ui and "doc_ref" not in st.session_state:
        ...
        st.success("Your document has been uploaded to gemini-3-flash-preview.")

    if "messages" not in st.session_state:
        st.session_state.messages = [] # creating a list of dictionaries where each dictionary is a message
        # containing role and content

    for msg in st.session_state.messages: # iterate through the list of dictionaries
        with st.chat_message(msg["role"]): # create a div/dialogue block for each role
